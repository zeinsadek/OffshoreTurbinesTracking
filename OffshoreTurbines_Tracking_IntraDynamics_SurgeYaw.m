...
for t = 1:length(centers)
    ...
    yaw = tracking.(spacing).(wave)(turbine).yaw_kal;
    
    % Remove mean
    surge = surge - mean(surge, 'all', 'omitnan');
    yaw = yaw - mean(yaw, 'all', 'omitnan');

    % Normalize by wave input
    normalized_surge = surge / amplitude;
    normalized_yaw = deg2rad(yaw) / steepness;
    ...
    skip = 1;
    hold on
    scatter(normalized_surge(1:skip:end), normalized_yaw(1:skip:end), 20, 'o', 'filled', ...
...
ylabel(tile, 'Normalized Yaw: $\psi \mathbin{/} ak$', 'interpreter', 'latex')
...
for t = 1:length(centers)
    ...
    for s = 1:length(farm_spacings)
        ...
        normalized_yaw_all = [normalized_yaw_all; ys(good)];
...
for t = 1:length(centers)
    ...
    A = pca_principal_axes_2d(normalized_surge_all(:), normalized_yaw_all(:));
