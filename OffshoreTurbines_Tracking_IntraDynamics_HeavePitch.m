...
for t = 1:length(centers)
    ...
    % Get signals
    turbine = centers(t);
    heave = tracking.(spacing).(wave)(turbine).z_kal;
    pitch = tracking.(spacing).(wave)(turbine).pitch_kal;
    
    % Remove mean
    heave = heave - mean(heave, 'all', 'omitnan');
    pitch = pitch - mean(pitch, 'all', 'omitnan');

    % Normalize by wave input
    normalized_heave = heave / amplitude;
    ...
    skip = 1;
    hold on
    scatter(normalized_heave(1:skip:end), normalized_pitch(1:skip:end), 20, 'o', 'filled', ...
...
xlabel(tile, 'Normalized Heave: $z \mathbin{/} a$', 'interpreter', 'latex')
...
for t = 1:length(centers)
    ...
    for s = 1:length(farm_spacings)
        ...
        heave = tracking.(farm_spacing).(wave)(turbine).z_kal;
        pitch = tracking.(farm_spacing).(wave)(turbine).pitch_kal;

        % Remove mean
        heave = heave - mean(heave, 'omitnan');
        ...
        xs = normalized_heave(1:skip:end);
        ...
        normalized_heave_all = [normalized_heave_all; xs(good)];
...
for t = 1:length(centers)
    ...
    A = pca_principal_axes_2d(normalized_heave_all(:), normalized_pitch_all(:));
...
figure('color', 'white')
tile = tiledlayout(1,4);
sgtitle(sprintf('%s Floating Wind Farm: Principal Axes Explained Variance\nHeave-Pitch', farm_arrangement), 'Interpreter', 'latex')
% Loop through rows of turbines
for t = 1:4
    turbine = centers(t);
