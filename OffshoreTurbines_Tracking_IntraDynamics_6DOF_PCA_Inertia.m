...
% Normalize
% Estimated total mass and inertias about the SAME reference point
...
% Radii of gyration [m]
rx = sqrt(Ixx / mass_total);   % for roll
ry = sqrt(Iyy / mass_total);   % for yaw
rz = sqrt(Izz / mass_total);   % for pitch

% Build inertia-weighted displacement coordinates
% Resulting units are all meters
surge_w = surge;
heave_w = heave;
sway_w  = sway;

roll_w  = rx * deg2rad(roll);   % roll about x
pitch_w = rz * deg2rad(pitch);  % pitch about z
yaw_w   = ry * deg2rad(yaw);    % yaw about y

% Assemble data matrix: rows = time, cols = DOFs
X = [surge_w(:), heave_w(:), sway_w(:), roll_w(:), pitch_w(:), yaw_w(:)];

% Remove mean before SVD/PCA
...
ylim([-0.02, 0.02])
xlabel(tile, 'Time [s]')
ylabel(tile, 'Gyration Displacement [m]')
