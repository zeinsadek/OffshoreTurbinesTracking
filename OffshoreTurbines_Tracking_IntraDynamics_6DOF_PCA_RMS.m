...
% Normalize
surge_norm = surge / rms(surge);
heave_norm = heave / rms(heave);
sway_norm  = sway  / rms(sway);


% Normalize rotations by displacement at hub, over platform length
roll_norm  = roll / rms(roll);
pitch_norm = pitch / rms(pitch);
yaw_norm   = yaw / rms(yaw);
...
h(1) = nexttile;
plot(time, surge_norm - mean(surge_norm))
ax = gca;
...
ylim([-4, 4])
xlabel(tile, 'Time [s]')
ylabel(tile, 'RMS Scaled Signals [~]')
