...
% Rotations (degrees)
roll  = deg2rad(tracking.(farm_spacing).(wave)(turbine).roll_kal);
pitch = deg2rad(tracking.(farm_spacing).(wave)(turbine).pitch_kal);
yaw   = deg2rad(tracking.(farm_spacing).(wave)(turbine).yaw_kal);

% Example estimated inertias [kg m^2]
mass_total = 0.15537;      % kg
Ixx = 5.56e-4;   % roll about x
Iyy = 4.62e-4;   % yaw  about y
Izz = 5.51e-4;   % pitch about z


% Velocities
surge_dot = gradient(surge, dt);
heave_dot = gradient(heave, dt);
sway_dot  = gradient(sway,  dt);

roll_dot  = gradient(roll,  dt);   % rad/s
pitch_dot = gradient(pitch, dt);   % rad/s
yaw_dot   = gradient(yaw,   dt);   % rad/s

% Kinetic-energy weighting
% units become sqrt(J) in a loose sense, but that is fine for SVD
surge_ke = sqrt(mass_total) * surge_dot;
heave_ke = sqrt(mass_total) * heave_dot;
sway_ke  = sqrt(mass_total) * sway_dot;

roll_ke  = sqrt(Ixx) * roll_dot;
pitch_ke = sqrt(Izz) * pitch_dot;
yaw_ke   = sqrt(Iyy) * yaw_dot;

% Assemble matrix: rows = time, cols = DOFs
X_ke = [surge_ke(:), heave_ke(:), sway_ke(:), ...
        roll_ke(:), pitch_ke(:), yaw_ke(:)];

% Remove mean
X_ke_mean = mean(X_ke, 1, 'omitnan');
X = X_ke - X_ke_mean;

figure('color', 'white')
tile = tiledlayout(6,1);
sgtitle('Kinetic Energy Scaling')
...
linkaxes(h, 'xy')
xlabel(tile, 'Time [s]')
ylabel(tile, '$\sqrt{Kinetic\,Energy}$', 'interpreter', 'latex')
